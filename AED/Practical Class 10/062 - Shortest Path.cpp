...
class Graph {
  ...
  public:
    ...
    std::list<int> shortestPath(int a, int b) {
        // Clear every info
        for (int i = 1; i <= n; i++) nodes[i].visited = false;
        for (int i = 1; i <= n; i++) nodes[i].previous = 0;
        for (int i = 1; i <= n; i++) nodes[i].distance = -1;
        ...
        while (!q.empty()) {
            ...
            for (auto edge : nodes[father].adj) {
                ...
                // Add info and push to queue
                if (nodes[child].visited == false) {
                    nodes[child].visited  = true;
                    nodes[child].previous = father;
                    nodes[child].distance = nodes[father].distance + 1;
                    q.push(child);
                }

                // Update previous if it shrinks
                else if (nodes[child].distance == nodes[father].distance + 1)
                    nodes[child].previous = std::min(nodes[child].previous, father);
            }
        }

        // No path found
        std::list<int> l;
        if (nodes[b].previous == 0) return l;

        // List path
        l.push_front(b);
        int v = b;
        while (v != a) l.push_front((v = nodes[v].previous));
        return l;
    }
};
