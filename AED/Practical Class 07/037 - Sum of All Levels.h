...
template <class T> class BTree {
  ...
  public:
    ...
    std::vector<int> sumLevels() {
        int              levels = height() + 1;
        std::vector<int> count(levels);
        sumLevels(count, this->root, 0);
        return count;
    }

    void sumLevels(std::vector<int> &count, Node *n, int level) {
        if (n == nullptr) return;
        count[level] += n->value;
        sumLevels(count, n->left, level + 1);
        sumLevels(count, n->right, level + 1);
    }
};
