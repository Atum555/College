...
class Graph {
  ...
  public:
    ...
    // Returns false if it's not bipartite, true otherwise
    bool bipartite_dfs(int i) {
        // Mark node as visited
        this->nodes[i].visited = true;

        // Loop through it's children
        for (auto edge : this->nodes[i].adj) {
            // If not yet visited assign the opposite side
            if (this->nodes[edge.dest].visited == false) {
                this->nodes[edge.dest].side = not this->nodes[i].side;

                // Do the same for their children
                if (not bipartite_dfs(edge.dest)) return false;
            }

            // If visited check if the side it's valid
            else if (this->nodes[edge.dest].side != not this->nodes[i].side)
                return false;
        }
    ...
    bool isBipartite() {
        ...
        for (int i = 1; i <= this->n; i++) this->nodes[i].visited = false;
        // PUT first node in a random side, say RIGHT
        this->nodes[1].side = RIGHT;

        // Separate remaining nodes starting on from the first
        return bipartite_dfs(1);
    }
};
