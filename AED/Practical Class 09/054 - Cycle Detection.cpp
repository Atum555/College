...
class Graph {
  ...
  public:
    ...
    bool cycle_dfs(int i) {
        // Mark as in current search
        this->nodes[i].color = GRAY;

        // Loop through it's children
        for (auto edge : this->nodes[i].adj) {
            switch (this->nodes[edge.dest].color) {
            case GRAY: return true;
            case WHITE:
                if (cycle_dfs(edge.dest)) return true;
            }
        }

        // Mark as search
        this->nodes[i].color = BLACK;
        return false;
    }

    bool hasCycle() {
        // Mark all as unvisited
        for (int i = 1; i <= this->n; i++) this->nodes[i].color = WHITE;

        // Loop through each non visited node
        for (int i = 1; i <= this->n; i++)
            if (this->nodes[i].visited == false)
                if (cycle_dfs(i)) return true;
