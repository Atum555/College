...
template <class T> class SinglyLinkedList {
  ...
  public:
    ...
    void insert(int pos, const T &v) {
        ...
        // Already implemented
        if (pos == 0) this->addFirst(v);
        else if (pos == length) this->addLast(v);

        // In the middle
        else {
            Node<T> *n = this->first;

            // n is the one before the desired position
            while (pos-- > 1) n = n->getNext();

            // Insert new node after n
            n->setNext(new Node<T>(v, n->getNext()));
