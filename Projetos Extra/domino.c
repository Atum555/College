...
void pTable(struct domino t[], int r) {
    printf("Mesa: ");
    for (int i = 0; i < r - 1; i++) { printf("%d%d ", t[i].n1, t[i].n2); }
    printf("\n");
}
...
void pPlaPoints(struct player p) {
    printf("%s ", p.name);
    int c = 0;
    for (int i = 0; i < p.handS; i++) { c += p.hand[i].n1 + p.hand[i].n2; }
    printf("%d pontos.\n", c);
}
            ...
            for (int i = 0; i < 4; i++) {
                for (int j = 0; j < 7; j++) {
                    if (theGame.players[i].hand[j].n1 == 6 && theGame.players[i].hand[j].n2 == 6) {
                        theGame.pTurn = i;
                        addDtoTable(
                            theGame.table, theGame.round, theGame.players[i].hand[j], false, false
                        );
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (int j = 0; j < theGame.players[(theGame.pTurn + i) % 4].handS; j++) {
                ...
                if (theGame.players[(theGame.pTurn + i) % 4].hand[j].n1 == first) {
                    ...
                    addDtoTable(
                        theGame.table, theGame.round,
                        theGame.players[(theGame.pTurn + i) % 4].hand[j], false, true
                    );
                    int n1 = theGame.players[(theGame.pTurn + i) % 4].hand[j].n1;
                    int n2 = theGame.players[(theGame.pTurn + i) % 4].hand[j].n2;
                    printf(
                    ...
                    addDtoTable(
                        theGame.table, theGame.round,
                        theGame.players[(theGame.pTurn + i) % 4].hand[j], false, false
                    );
                    int n1 = theGame.players[(theGame.pTurn + i) % 4].hand[j].n1;
                    int n2 = theGame.players[(theGame.pTurn + i) % 4].hand[j].n2;
                    printf(
                        "-> %s joga %d%d no fim!\n", theGame.players[(theGame.pTurn + i) % 4].name,
                        n1, n2
                    );
                    removeDfromP(theGame.players, theGame.pTurn + i, j);
                    play = true;
                    ...
                    addDtoTable(
                        theGame.table, theGame.round,
                        theGame.players[(theGame.pTurn + i) % 4].hand[j], true, false
                    );
                    int n1 = theGame.players[(theGame.pTurn + i) % 4].hand[j].n1;
                    int n2 = theGame.players[(theGame.pTurn + i) % 4].hand[j].n2;
                    printf(
